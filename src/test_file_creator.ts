...
export function createTestFile(
  ...
  if (fileOperations.isPathInLibFolder(originalFilePath)) {
    ...
    fs.mkdir(path.dirname(pathOfTestFile), { recursive: true }, (err) => {
      ...
      var testFileContent = getTestFileContent(
        getPackagePath(originalFilePath),
        testGroupName
      );

      //TODO: Check whether file already exists to prevent overwriting!
    ...
    vscode.window.showErrorMessage(
      "File must be inside of /lib folder of your workspace folder"
    );
  }
}
