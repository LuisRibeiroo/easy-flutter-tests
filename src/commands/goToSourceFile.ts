...
export function activate(context: vscode.ExtensionContext) {
  let disposableGoToSource = vscode.commands.registerCommand(
    "easy-tests.goToSourceFile",
    async (args) => {
      var path = vscode.window.activeTextEditor?.document.uri.path;

      if (path !== undefined) {
        var searchResultPath = fileOperations.searchSourceFilePath(
          fileOperations.getNameOfSourceFile(path)
        );
